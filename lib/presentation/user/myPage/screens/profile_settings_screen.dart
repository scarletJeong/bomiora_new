...
class _ProfileSettingsScreenState extends State<ProfileSettingsScreen> {
  ...
  DateTime? get _nextNicknameChangeDate {
    ...
    return DateTime(
        last.year, last.month + _nicknameChangeIntervalMonths, last.day);
  }
  ...
  ButtonStyle _confirmOtpButtonStyle() {
    ...
    return ElevatedButton.styleFrom(
      backgroundColor: pink ? const Color(0xFFFF5A8D) : const Color(0xFFD2D2D2),
  ...
  Future<void> _requestContactChangeOtp() async {
    ...
    final phoneDigits = _digitsOnly(_enteredPhoneDigits);
    if (name.isEmpty) {
      if (!mounted) return;
      setState(() {
        _contactOtpErrorText = '이름 정보가 없습니다.';
      });
      return;
    }
    if (phoneDigits.length < 10) {
      if (!mounted) return;
      setState(() {
        _contactOtpErrorText = '연락처를 올바르게 입력해 주세요.';
      });
      return;
    }
    final phoneApi = _formatPhoneForApi(phoneDigits);
    ...
    if (send['success'] != true) {
      setState(() {
        _contactOtpErrorText = send['error']?.toString() ?? '인증번호 발송에 실패했습니다.';
      });
      return;
    }
  ...
  Future<void> _confirmContactChangeOtp() async {
    ...
    if (verify['success'] != true) {
      setState(() {
        _contactOtpErrorText = verify['error']?.toString() ?? '인증에 실패했습니다.';
      });
      return;
    }
    ...
    setState(() {
      _contactPhoneVerified = true;
      _originalPhoneDigits = _digitsOnly(_enteredPhoneDigits);
      _contactOtpToken = null;
      _secondsLeft = 0;
      _contactOtpErrorText = null;
  ...
  Widget _buildPersonalInfoBody() {
    ...
    Widget buildPhoneRow(double maxWidth) {
      final idealW = healthDp(context, 62.3);
      final gap = healthDp(context, 5);
      final hyphenW = healthDp(context, 10);
      final fixedExtra = 4 * gap + 2 * hyphenW;
      final segmentW = math.min(idealW, (maxWidth - fixedExtra) / 3);
  ...
  Widget _buildProfileFormContent({
    ...
    return Column(
      ...
      children: [
        ...
        GestureDetector(
          onTap:
              !_canChangeNicknameNow() ? _showNicknameChangeLimitToast : null,
          child: AbsorbPointer(
        ...
        Row(
          children: [
            ...
            SizedBox(
              height: healthDp(context, 40),
              child: ElevatedButton(
                onPressed: _contactOtpSending ? null : _requestContactChangeOtp,
        ...
        if (_contactOtpErrorText != null &&
            _contactOtpErrorText!.isNotEmpty) ...[
...
class _InputBoxState extends State<_InputBox> {
  ...
  Widget build(BuildContext context) {
    ...
    final borderColor =
        isError ? widget.borderColor : (_focused ? _pink : widget.borderColor);
    return Focus(
